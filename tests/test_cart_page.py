...
def test_added_product_is_displayed_with_correct_content_on_cart_page(
...
def test_cart_state_persists_after_logout_and_relogin(
    ...
    cart_page_after_relogin = relogged_inventory_page.open_cart()

    expect(cart_page_after_relogin.get_product_item_by_name(tested_product_name)).to_be_visible()
...
def test_all_added_products_are_displayed_with_correct_content_on_cart_page(
    logged_in_inventory_page: InventoryPage,
    _case_id: str,
):
    ...
    expect(cart_page.get_product_item_or_items()).to_have_count(len(LIST_OF_PRODUCTS))
...
@pytest.mark.parametrize(
    "product",
    LIST_OF_PRODUCTS,
    ids=[f"TC-CART-011-{product['product_id']}" for product in LIST_OF_PRODUCTS],
)
def test_all_products_can_be_removed_from_cart(
    logged_in_inventory_page: InventoryPage,
    product,
):
...
def test_checkout_button_opens_checkout_information_page_with_product_in_cart(
    cart_page_with_one_product: tuple[
        CartPage,
        dict[str, str],
    ],
    _case_id: str,
